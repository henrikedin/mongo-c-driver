...
TAR_CFILES=`tar --wildcards -tf mongo-c-driver-*.tar.gz 'mongo-c-driver-*/src/libmongoc/src/mongoc/mongoc/*.c' | cut -d / -f 4 | sort`
SRC_CFILES=`echo src/libmongoc/src/mongoc/mongoc/*.c | xargs -n 1 | cut -d / -f 3 | sort`
